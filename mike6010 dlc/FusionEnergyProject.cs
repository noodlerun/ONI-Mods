...
using static FusionEnergyProject.NuclearBomb;
using static SimMessages;


namespace FusionEnergyProject
{
    ...
    public class DLC_Entry : UserMod2
    {
        ...
        public override void OnLoad(Harmony harmony)
        {
            ...
            FusionReactorSettings.Load(System.IO.Path.Combine(modPath, "config.txt"));
    ...
    public static class FusionReactorSettings
    {
        private const string LogPrefix = "[mike6010 DLC][FusionReactorConfig]";

        public static float BaseSelfHeatDtuPerSecond { get; private set; } = 10000000f;
        public static float MaxTritiumKgPerSecond { get; private set; } = 1f;
        public static float MinTritiumKgPerSecond { get; private set; } = 0.1f;
        public static float LowTritiumWarningTemperatureCelsius { get; private set; } = 500f;
        public static float WasteReleaseInterval { get; private set; } = 4f;
        public static float NuclearWasteOutputRatio { get; private set; } = 0.9f;
        public static float NuclearWasteTemperature { get; private set; } = 9900f;

        public static void Load(string configPath)
        {
            if (!System.IO.File.Exists(configPath))
            {
                Debug.LogWarning($"{LogPrefix} config.txt not found at '{configPath}', using default values.");
                return;
            }

            try
            {
                foreach (string rawLine in System.IO.File.ReadAllLines(configPath))
                {
                    ...
                    {
                        Debug.LogWarning($"{LogPrefix} invalid float value for '{key}': '{valueText}', keeping current value.");
                        continue;
                    }

                    switch (key)
                    {
                        case "BASE_SELF_HEAT_DTU_PER_SECOND":
                            BaseSelfHeatDtuPerSecond = value;
                            break;
                        case "MAX_TRITIUM_KG_PER_SECOND":
                            MaxTritiumKgPerSecond = value;
                            break;
                        case "MIN_TRITIUM_KG_PER_SECOND":
                            MinTritiumKgPerSecond = value;
                            break;
                        case "LOW_TRITIUM_WARNING_TEMPERATURE_CELSIUS":
                            LowTritiumWarningTemperatureCelsius = value;
                            break;
                        case "WASTE_RELEASE_INTERVAL":
                            WasteReleaseInterval = value;
                            break;
                        case "NUCLEAR_WASTE_OUTPUT_RATIO":
                            NuclearWasteOutputRatio = value;
                            break;
                        case "NUCLEAR_WASTE_TEMPERATURE":
                            NuclearWasteTemperature = value;
                            break;
                    }
                }
            }
            ...
            {
                Debug.LogWarning($"{LogPrefix} failed to read '{configPath}', using default values. Exception: {e}");
            }
        }
    }
    ...
    public static class BUILDINGS
    {
        public static class PREFABS
        {
            ...
            public static class GASCENTRIFUGE
            {
                ...
                public static LocString DESC = (LocString)"Extracting tritium from hydrogen.";
                public static LocString EFFECT = (LocString)"Extracting 5% <link=\"TRITIUM\">tritium gas</link> from 100% <link=\"HYDROGEN\">hydrogen gas</link>.";
            ...
            public static class FUSIONREACTOR
            {
                ...
                public static LocString NOTIFICATION_TITLE = (LocString)"Tritium Too Cold";
                public static LocString NOTIFICATION1 = (LocString)"Stored tritium is below {0}. Current temperature: {1}";
            }
    ...
    public class Db_Initialize_Patch
    {
        ...
        public static void Prefix()
        {
            if (DLC_Entry.dic != null)
            {
                ...
                DLC_Entry.dic.TryGetValue("BUILDINGS.PREFABS.FUSIONREACTOR.NOTIFICATION_TITLE", out string notificationTitle);
                DLC_Entry.dic.TryGetValue("BUILDINGS.PREFABS.FUSIONREACTOR.NOTIFICATION1", out string notificationText);

                BUILDINGS.PREFABS.FUSIONREACTOR.NOTIFICATION_TITLE = (LocString)notificationTitle;
                BUILDINGS.PREFABS.FUSIONREACTOR.NOTIFICATION1 = (LocString)notificationText;
            ...
            StringUtils.AddStatusItemStrings("FusionReactorTritiumTooCold", "BUILDING", (string)BUILDINGS.PREFABS.FUSIONREACTOR.NOTIFICATION_TITLE, (string)BUILDINGS.PREFABS.FUSIONREACTOR.NOTIFICATION1);
        }
    ...
    public class GasCentrifugeConfig : IBuildingConfig
    {
        ...
        public const float CONSUMPTION_RATE = 0.05f; // kg/s
        public override BuildingDef CreateBuildingDef()
        {
            int width = 4;
            int height = 4;
    ...
    public class FusionReactor : StateMachineComponent<FusionReactor.StatesInstance>, ISim1000ms, IGameObjectEffectDescriptor
    {
        ...
        private bool hasSentColdTritiumNotification;
        private float lowTemperatureTritiumSample = LowTritiumWarningTemperatureKelvin;
        private Notifier notifier;
        private Notification coldTritiumNotification;
        [MyCmpGet]
        ...
        private static float BaseSelfHeatDtuPerSecond => FusionReactorSettings.BaseSelfHeatDtuPerSecond;
        private static float MaxTritiumKgPerSecond => FusionReactorSettings.MaxTritiumKgPerSecond;
        private static float MinTritiumKgPerSecond => FusionReactorSettings.MinTritiumKgPerSecond;
        private static float LowTritiumWarningTemperatureCelsius => FusionReactorSettings.LowTritiumWarningTemperatureCelsius;
        private static float WasteReleaseInterval => FusionReactorSettings.WasteReleaseInterval;
        private static float NuclearWasteOutputRatio => FusionReactorSettings.NuclearWasteOutputRatio;
        private static float NuclearWasteTemperature => FusionReactorSettings.NuclearWasteTemperature;
        private static float LowTritiumWarningTemperatureKelvin => LowTritiumWarningTemperatureCelsius + KELVIN_OFFSET;
        ...
        protected override void OnSpawn()
        {
            ...
            notifier = gameObject.AddOrGet<Notifier>();
            worldPos = transform.GetPosition();
            cell = Grid.PosToCell(worldPos);
        ...
        protected override void OnPrefabInit()
        {
            ...
            lowTritiumTemperatureStatusItem.resolveTooltipCallback = (str, data) =>
            {
                ...
                str = str.Replace("{0}", GameUtil.GetFormattedTemperature(LowTritiumWarningTemperatureKelvin));
        ...
        public void Sim1000ms(float dt)
        {
            ...
            if (IsReactionBlockedByColdTritium())
            {
                SetHasFuel(false);
                SetHeatOutput(0f, 0f);
            ...
            if (consumedKg < MinTritiumKgPerSecond)
            {
                SetHasFuel(false);
                SetHeatOutput(0f, 0f);
            }
            ...
            {
            float ratio = Mathf.Clamp01(consumedKg / MaxTritiumKgPerSecond);
            float heatKW = BaseSelfHeatDtuPerSecond * ratio / DTU_PER_KW;
            ...
            StoreNuclearWaste(consumedKg * NuclearWasteOutputRatio);
                SetHasFuel(true);
            }

            wasteReleaseTimer += dt;
            if (wasteReleaseTimer >= WasteReleaseInterval)
            {
                ReleaseStoredNuclearWaste();
            }

            if(wasteReleaseTimer >= 6.0f)
            {
        ...
        private float ConsumeTritium()
        {
            ...
            if (availableKg < MinTritiumKgPerSecond)
                return 0f;

            float consumedKg = Mathf.Min(availableKg, MaxTritiumKgPerSecond);
        ...
        {
            return TritiumStorage != null && TritiumStorage.GetMassAvailable(TritiumID) >= MinTritiumKgPerSecond;
        }
        ...
        private void SampleLowTritiumTemperature()
        {
            ...
            hasLowTemperatureTritiumWarning = TryGetColdestStoredTritiumTemperature(out currentTemperature) && currentTemperature < LowTritiumWarningTemperatureKelvin;
            lowTemperatureTritiumSample = hasLowTemperatureTritiumWarning ? currentTemperature : LowTritiumWarningTemperatureKelvin;
        }

        private bool IsReactionBlockedByColdTritium()
        {
        ...
        {
            bool shouldPlayOn = operational != null
                && operational.IsOperational
                && !hasLowTemperatureTritiumWarning
                && (HasFuel || HasEnoughStoredTritium());
            return shouldPlayOn;
        ...
        private void StoreNuclearWaste(float mass)
        {
            if (mass <= 0f)
                return;
            nuclearWasteStorage.AddLiquid(SimHashes.NuclearWaste, mass, NuclearWasteTemperature, byte.MaxValue, 0, false, false);
        }
        ...
        private void ReleaseStoredNuclearWaste()
        {
            ...
            FallingWater.instance.AddParticle(cell, nuclearWasteIdx, wasteKg, NuclearWasteTemperature, byte.MaxValue, 0);
        ...
        private void SetHeatOutput(float heatKW, float tritiumKgPerSecond)
        {
            ...
            UpdateRadiationEmitter(operational != null && operational.IsOperational && tritiumKgPerSecond >= MinTritiumKgPerSecond);
        }
        ...
        private void UpdateColdTritiumNotification()
        {
            if (!hasLowTemperatureTritiumWarning)
            {
                hasSentColdTritiumNotification = false;
                ClearColdTritiumNotification();
                return;
            }

            if (operational == null || !operational.IsOperational || hasSentColdTritiumNotification || notifier == null)
                return;

            coldTritiumNotification = CreateColdTritiumNotification();
            notifier.Add(coldTritiumNotification);
            hasSentColdTritiumNotification = true;
        }

        private Notification CreateColdTritiumNotification()
        {
            return new Notification(
                (string)BUILDINGS.PREFABS.FUSIONREACTOR.NOTIFICATION_TITLE,
                NotificationType.Neutral,
                (notificationList, data) => string.Format(
                    (string)BUILDINGS.PREFABS.FUSIONREACTOR.NOTIFICATION1,
                    GameUtil.GetFormattedTemperature(LowTritiumWarningTemperatureKelvin),
                    GameUtil.GetFormattedTemperature(lowTemperatureTritiumSample)),
                clear_on_click: true);
        }

        private void ClearColdTritiumNotification()
        {
            if (coldTritiumNotification == null)
                return;

            coldTritiumNotification.Clear();
            coldTritiumNotification = null;
        }
        ...
        private void SetFuelStatusMeter(float tritiumKgPerSecond)
        {
            ...
            if (tritiumKgPerSecond >= MinTritiumKgPerSecond)
            {
                int displayedUnits = Mathf.Clamp(Mathf.RoundToInt(Mathf.Clamp(tritiumKgPerSecond, MinTritiumKgPerSecond, MaxTritiumKgPerSecond) * 10f), 1, 10);
        ...
        public List<Descriptor> GetDescriptors(GameObject go)
        {
            ...
            string formattedHeatEnergy = GameUtil.GetFormattedHeatEnergy(BaseSelfHeatDtuPerSecond);
